...
entrega, y desde esa misma tarde la 48, que es la misma dibujada NEGRA de
origen —que es como es la correa de verdad— y con la tabla en cuatro
columnas. Viene dibujada entera —hebilla, trabilla, tabla de tiempos sin
...
CORREAS = {
    ...
    # ⚠️ VA EN NEGRO, no en el azul con el que llegó dibujada (Óscar,
    ...
    # pantalla.
    # Y ES LA 48, DIBUJADA NEGRA DE ORIGEN, no la 47 recoloreada: Óscar la
    # mandó esa misma tarde, con el mismo lienzo, las mismas tiras y las
    # mismas puntas —343 y 359 px, clavadas—, así que entra sin tocar nada
    # del montaje. Es más negra de verdad (valor 0,145 contra 0,266 de la
    # recoloreada) y trae la tabla de tiempos en CUATRO columnas en vez de
...
def _ruta(f):
    ...
    para los dos."""
    return f if os.path.isabs(f) else ENTREGA + f


def alfa(f, u=128):
    return np.asarray(Image.open(_ruta(f)).convert('RGBA'))[:, :, 3] > u
...
def punta_del_indice(fondo=120.0):
    ...
    for f in sorted(ESFERAS.values()):
        a = np.asarray(Image.open(_ruta(f)).convert('RGBA'))
        al = a[:, :, 3] > 128
        rgb = a[:, :, :3].astype(int)
...
def pon_correa(f, s, cx, centro, baja=0, sube=0):
    ...
    que sobra."""
    im = Image.open(_ruta(f)).convert('RGBA')
