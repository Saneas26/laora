...
(function () {
  ...
  /* La factura la dibuja `factura.js`, que es el mismo documento que
     ve el cliente en su cuenta. Aquí solo se piden los números. */
  function imprimirFactura(id) {
    api('factura', { id: id }).then(function (d) {
      if (!d.pedido.factura_numero) { avisar('Ese pedido todavía no tiene factura.', true); return; }
      if (!laoraFactura.abrir(d)) avisar('El navegador ha bloqueado la ventana de la factura.', true);
    }).catch(function (e) { avisar(e.message, true); });
  }
